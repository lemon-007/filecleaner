...
impl Chunk {
    ...
    fn inform(&mut self) {
        ...
        println!("Chunk size: {} | type: {} | required: {}", 
            self.length,
            ctype_ext,
            self.required
        );
...
fn main() -> Result<(), Error>{
    ...
    let file = File::open(Path::new("/home/nate/testpictures/smartimg2.png"))?;
    let length_file = file.metadata().unwrap().len();
    let rm_unwanted = remove_chunks_prompt();

    // Removing chunks can't be done in place, so we would have to make a new file.
...
fn is_png(reader: &mut BufReader<File>) -> Result<(), Error> {
    ...
    let buf_vect = first_8.to_vec();

    if buf_vect == [137, 80, 78, 71, 13, 10, 26, 10] {
        Ok(())
    } else {
        Err(Error::new(std::io::ErrorKind::InvalidInput, "Not a PNG."))
    }
}
...
fn remove_chunks_prompt() -> bool {
    println!("Remove all unwanted chunks? [Y/n]: ");
    ...
    let choice = match rm.trim().to_lowercase().as_str() {
        ...
        "n" => false,
        _ => {
            print!("Input either invalid or unable to be read. Continuing with default, no chunks will be removed.");
            false
        }
    };
